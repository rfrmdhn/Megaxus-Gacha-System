...
import { AdminEvent } from "./EventFormDialog";
...
export function EventItemsDialog({
  ...
  async function addItem(e: FormEvent) {
    ...
    setSaving(true);
    try {
      await apiFetch(`/admin/events/${event.id}/items`, {
        method: "POST",
        body: JSON.stringify({ name: itemName, rarity, dropRate: parseFloat(dropRate) }),
      });
      setItemName("");
      setRarity("");
      setDropRate("");
      ...
      <div className="flex flex-col gap-4">
        ...
        <form onSubmit={addItem} className="flex flex-wrap items-end gap-2 border-t border-black/10 pt-4">
          <input
            ...
            className="rounded-lg border border-black/15 px-2 py-1.5 text-sm outline-none focus:border-brand-purple focus:ring-2 focus:ring-brand-purple/30"
          />
          <input
            required
            placeholder="Rarity"
