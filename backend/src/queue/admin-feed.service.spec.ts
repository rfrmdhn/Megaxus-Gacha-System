...
describe('AdminFeedService', () => {
  ...
  it('publishes events that are received by the stream', (done) => {
    ...
    service.stream().pipe(take(1)).subscribe((messageEvent) => {
  ...
  it('delivers multiple published events in order', (done) => {
    const events = [
      { userId: 'u1', userEmail: 'a@test.com', eventId: 'e1', eventName: 'E1', itemName: 'I1', rarity: 'common', createdAt: '2026-01-01' },
      { userId: 'u2', userEmail: 'b@test.com', eventId: 'e2', eventName: 'E2', itemName: 'I2', rarity: 'rare', createdAt: '2026-01-02' },
    ];

    const received: any[] = [];
    service.stream().pipe(take(2)).subscribe({
