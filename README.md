...
This is the **Linear Prefix Sum** algorithm (see [Key design decisions](#key-design-decisions) for why this over binary search or the Alias Method):
...
`GET /admin/history` (cursor-paginated) remains the mechanism for initial load and scrolling back — the SSE stream is purely "new since I started watching," with no replay of past events on connect. See [Key design decisions](#key-design-decisions) for the single-instance scale-out limitation.
...
- Sound files are **intentionally not committed to the repo** — Howler fails silently on a missing file, so the app degrades gracefully to no audio rather than erroring.
